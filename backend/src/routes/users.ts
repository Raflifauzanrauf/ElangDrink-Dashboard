...
function toSafeUser(u: User): SafeUser {
  ...
  return { id: u.id, email: u.email, name: u.name, role: u.role, roleId: u.roleId, permissions: role ? role.permissions : [], createdAt: u.createdAt, updatedAt: u.updatedAt };
}

router.get("/", authenticate, authorize("admin", "manager"), (req: Request, res: Response) => {
...
router.post("/", authenticate, authorize("admin"), auditMiddleware("create", "user", (req) => req.body.email), async (req: Request, res: Response) => {
...
router.put("/:id", authenticate, authorize("admin"), auditMiddleware("update", "user"), async (req: Request, res: Response) => {
...
router.delete("/:id", authenticate, authorize("admin"), auditMiddleware("delete", "user"), (req: Request, res: Response) => {
